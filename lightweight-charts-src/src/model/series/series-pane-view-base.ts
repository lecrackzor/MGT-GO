...
import { SeriesItemsIndexesRange, TimedValue, TimePointIndex, visibleTimedValues } from '../time-data';
...
export abstract class SeriesPaneViewBase<TSeriesType extends SeriesType, ItemType extends TimedValue, TRenderer extends IPaneRenderer> implements IUpdatablePaneView {
	...
	private _lastRightOffset: number = NaN;
	private _lastBarSpacing: number = NaN;
	private _lastVisibleLeft: TimePointIndex | null = null;
	private _lastVisibleRight: TimePointIndex | null = null;
	...
	private _makeValid(): void {
		...
		// When the user pans or zooms, bar coordinates must be recomputed so custom series (e.g. segment plots) don't stack at the edge.
		// Invalidate when rightOffset/barSpacing or the visible strict range change (pan/zoom may only change visible range).
		const rightOffset = timeScale.rightOffset();
		const barSpacing = timeScale.barSpacing();
		const visibleBars = timeScale.visibleStrictRange();
		const visibleLeft = visibleBars === null ? null : visibleBars.left();
		const visibleRight = visibleBars === null ? null : visibleBars.right();
		if (
			rightOffset !== this._lastRightOffset ||
			barSpacing !== this._lastBarSpacing ||
			visibleLeft !== this._lastVisibleLeft ||
			visibleRight !== this._lastVisibleRight
		) {
			this._invalidated = true;
		}
	...
	private _makeValidImpl(): void {
		...
		this._lastRightOffset = timeScale.rightOffset();
		this._lastBarSpacing = timeScale.barSpacing();
		this._lastVisibleLeft = visibleBars.left();
		this._lastVisibleRight = visibleBars.right();
