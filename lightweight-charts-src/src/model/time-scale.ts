...
export interface HorzScaleOptions {
	...
	/**
	 * Allow the visible range to extend past the first/last data point (show empty space at edges).
	 *
	 * @defaultValue `false`
	 */
	allowVisibleRangeOutsideData: boolean;
...
export class TimeScale<HorzScaleItem> implements ITimeScale {
	...
	/**
	 * Set right offset without triggering recalculate/repaint.
	 * Used when preserving visible range on new bar (shiftVisibleRangeOnNewBar: false) to avoid flicker:
	 * we apply compensation so the next paint uses the correct range, but we do not repaint until the application restores the range.
	 */
	public setRightOffsetQuiet(offset: number): void {
		this._visibleRangeInvalidated = true;
		this._rightOffset = offset;
		this._correctOffset();
	}
	...
	private _correctOffset(): void {
		if (this._options.allowVisibleRangeOutsideData) {
			return;
		}
