...
export interface ChartOptionsBase {
	...
	/**
	 * When to show series values on the price scale: last visible value, or value at crosshair.
	 *
	 * @defaultValue `'lastVisible'`
	 */
	priceScaleLabelsSource?: 'lastVisible' | 'crosshair';
...
export class ChartModel<HorzScaleItem> implements IDestroyable, IChartModelBase {
	...
	public cursorUpdate(): void {
		const level = this._options.priceScaleLabelsSource === 'crosshair'
			? InvalidationLevel.Light
			: InvalidationLevel.Cursor;
		this._invalidate(new InvalidateMask(level));
	}
	...
	public updateCrosshair(): void {
		...
		if (this._options.priceScaleLabelsSource === 'crosshair') {
			this._serieses.forEach((s: Series<SeriesType>) => s.invalidatePriceAxisViews());
		}
	}
	...
	public updateTimeScale(newBaseIndex: TimePointIndex | null, newPoints?: readonly TimeScalePoint[], firstChangedPointIndex?: number): void {
		...
		if (visibleBars !== null && oldFirstTime !== null && newFirstTime !== null) {
			...
			if (isSeriesPointsAddedToRight && !needShiftVisibleRangeOnNewBar) {
				...
				this._timeScale.setRightOffsetQuiet(this._timeScale.rightOffset() - compensationShift);
			}
		}
