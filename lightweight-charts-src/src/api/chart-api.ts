...
export class ChartApi<HorzScaleItem> implements IChartApiBase<HorzScaleItem>, DataUpdatesConsumer<SeriesType, HorzScaleItem> {
	...
	private _sendUpdateToChart(update: DataUpdateResponse): void {
		...
		// When shiftVisibleRangeOnNewBar is false, the app will restore the visible range; skip recalc here so the next paint uses the restored range and avoids flicker.
		if (model.timeScale().options().shiftVisibleRangeOnNewBar) {
