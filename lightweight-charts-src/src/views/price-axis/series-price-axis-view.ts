...
export class SeriesPriceAxisView extends PriceAxisView {
	...
	protected _updateRendererData(
		...
		const labelsSource = model.options().priceScaleLabelsSource ?? 'lastVisible';
		const crosshair = model.crosshairSource();
		const useCrosshairValue = labelsSource === 'crosshair' && crosshair.visible() && typeof source.valueDataAt === 'function';

		const lastValueData = useCrosshairValue
			? source.valueDataAt!(crosshair.appliedIndex())
			: source.lastValueData(false);
